(function () {
  ...
  function labelFor(item) {
    ...
    if (item.role === "student") {
      ...
      return `${name || "Ученик"} ${confidence}%${attention}`;
    }
  ...
  async function refreshState() {
    try {
      ...
      if (!smoothVideo && !smoothSyncVideo && !steppedAiVideo && (
      ...
      attentionChip.textContent = formatPercent(state.attention);
      detectionChip.textContent = `${state.people_count || 0} persons · ${state.phones_count || 0} phones`;
      renderBoxes(state);
      renderRecognitionList(state);
    } catch (error) {
      overlay.replaceChildren();
  ...
  video.addEventListener("loadedmetadata", refreshState);
